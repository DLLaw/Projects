    ...
    def __draw_board__(self):
        ...
        for x in range(BOARD_SIZE):
            pg.draw.line(
                ...
                (self.board_x_offset + CELL_SIZE // 2 + x * CELL_SIZE,
                 self.board_y_offset + BOARD_HEIGHT - CELL_SIZE // 2)
            )
            pg.draw.line(
                self._screen, GRID_COLOR,
                (self.board_x_offset + CELL_SIZE // 2, self.board_y_offset + CELL_SIZE // 2 + x * CELL_SIZE),
                (self.board_x_offset + BOARD_WIDTH - CELL_SIZE // 2,
                 self.board_y_offset + CELL_SIZE // 2 + x * CELL_SIZE)
            )
